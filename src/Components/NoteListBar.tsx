...
export const NoteListBar = () => {
  ...
  if (error) return <p className="text-red-500 px-4">{error as string}</p>;
  ...
  return (
    <>
      {!open && (
        ...
        >
          <Menu className="size-7"></Menu>
        </button>
      )}
     {open && (
  <aside
    className="fixed top-0 left-0 h-full w-60 z-50 p-5 shadow-2xl shadow-gray-900 bg-gradient-to-tl from-black to-sky-950"
  >
    <div className="pt-4">
      <button
        onClick={() => setOpen(!open)}
