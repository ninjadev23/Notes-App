...
const NoteComponent: React.FC = () => {
  ...
  return (
    <main className="mt-20 flex flex-col w-screen h-screen items-center">
      {note && (
        <>
          {savingMessage && (
          ...
          {panelOptions && (
            <ul className="right-20 top-30 shadow-2xl shadow-gray-900 p-1 m-2 rounded-lg gap-2 fixed z-index bg-sky-950 w-32 h-24 flex flex-col items-center justify-center">
              ...
              <li onClick={() => setDisplayInputTag(!displayInputTag)} className="hover:text-white/40 hover:cursor-pointer">
                Add Tag
              </li>
            </ul>
          )}
          ...
          <div className="flex gap-2 m-2">
            {note.tags.map((tag, i) => (
              ...
              >
                {tag}
                <X onClick={()=>handleTagDelete(tag)} className="hover: cursor-pointer" size={20} />
              </div>
            ))}
            {displayInputTag && (
            ...
            <button
              ...
              className="p-1 bg-black/30 hover:cursor-pointer hover:bg-black/40"
            >
              Add Tag
            </button>
          </div>
